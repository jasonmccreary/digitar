...
class FileController extends BaseController {
	...
	public function showFiles($fid,$ajax = false) {
		if (!Auth::check() || Auth::user()->rights != 1) { return Redirect::to('/'); }
		View::share('fid',$fid);
		...
		if (!is_numeric($fid)) {
			...
			if (Folderright::where('fid','=',$fid)->where('uid','=',Auth::user()->id)->count() <= 0) { return Redirect::to('403'); } // IF A USER IS NOT AUTHORIZED TO VIEW THE FOLDER
		...
		if ($ajax == false) {
			return View::make('users.files', array(
	        	'title' => $title,
	        	'noGrid' => 'true'
	        ));
	    	...
	    	return Response::json(array('data' => $jsonfiles), 200);
	    }

	}

	static public function getOngeboekt() {
		if (!Auth::check() || Auth::user()->rights != 1) { return Response::json(array('data' => []), 200); }
	...
	static public function addFile($name, $filename,$cid, $fileContents = '') {
	...
	static public function markBooked($fids) {
	...
	static public function moveToFolder($fids,$foid) {
		$fc = new FoldersController();
		foreach ($fids as $fid => $v) {
			$f = new Files();
			$f = $f->where('id','=',$fid)->where('cid','=',Auth::user()->cid);
	...
	static public function downloadFiles($fids) {
		$first = $fids;
		reset($first);
		$first = key($first);
	...
	static public function combineFiles($fids) {
		$first = $fids;
		reset($first);
		$first = key($first);
	...
	static public function splitFiles($fid) {
		$f = new Files();
		$f = $f
			->where('id', '=', $fid)
			->where('cid', '=', Auth::user()->cid);
	...
	public function upload() {
	    ...
	    $v1 = Validator::make($input, array('file' => 'mimes:jpg,jpeg,png,pdf|max:10240'));
	    if (!$v1->fails()) {
			$u = new User();
			$organization = $u
						->where('id', '=', Auth::user()->oid)
			...
			if( $upload_success ) {
				...
				if (in_array(Input::file('file')->getClientOriginalExtension(), array('png','jpg','jpeg'))) {
				...
				if (Input::get('filename')) { $savefilename = Input::get('filename'); }else { $savefilename = $file->getClientOriginalName(); }
		...
		$v2 = Validator::make($input, array('file' => 'mimes:xml,csv,doc,docx,xls,xlsx,swi,txt,mut,bacpac,ssb,|max:10240'));
	    if (!$v2->fails()) {
	    	$u = new User();
			$organization = $u
						->where('id', '=', Auth::user()->oid)
	...
	public function viewfile($fid) {
		...
		return View::make('users.viewfile', array(
        	'file' => $file
        ));
	}
	...
	public function loadpdf($fid) {
		...
		if(!$client->count()) { return Redirect::to('/user/folder/inbox'); }
		...
		if ($v->fails()) {
			...
			Mail::queue('emails.files', Input::all(), function($message) {
			    ...
			    foreach(Input::get('fileid') as $fileid => $filename) {
			    	...
			    	$message->attach($pathToFile, array('as' => $file->name.'.pdf'));
			    }
			});
	...
	static public function getFolderPath($folderid = false, $clientid = false, $type = 'uf',$oid = false) {
		if ($clientid == false) { $clientid = Auth::user()->cid; }
		if ($oid == false) { $oid = Auth::user()->oid; }
		$u = new User();
		$organization = $u
					->where('id', '=', $oid);
		if(!$organization->count()) { return Redirect::to('/user/folder/inbox'); }
		$client = $u
					->where('id', '=', $clientid)
					->where('oid', '=', $oid);
		if(!$client->count()) { return Redirect::to('/user/folder/inbox'); }
