...
Route::get('client', function() {
	...
	Route::get('client/users', function() {
        ...
        return View::make('client.users.overview', array(
        	'title' => 'Gebruikers',
        	'users' => $aUsers
        ));
	})->before('auth');
		...
		*/
		Route::get('client/user/add', function() {
			...
			return View::make('client.users.add', array(
				'title' => 'Gebruiker toevoegen',
				'sfolders' => $sfolders
			));
		})->before('auth');
		Route::post('client/user/add', array(
			'before' => 'auth',
		...
		*/
		Route::get('client/user/edit/{id}', function($id) {
			...
			return View::make('client.users.edit', array(
		...
		*/
		Route::get('client/user/delete/{id}', function($id) {
			$u = new User();
			$user = $u->find($id);
			return View::make('client.users.delete', array(
		...
		*/
		Route::get('client/user/credentials/{id}', function($id) {
			...
			$useremail = $u->email;
    		if (strlen($u->email) > 3) {
                Mail::send('emails.credentials', array('name'=>$u->name, 'username'=>$u->username, 'password'=>$u->password, 'client'=>$c->name), function($message) {
                    global $useremail;
	...
	*/
	 ...
	 Route::get('client/folders', array('as' => 'folders', function() {
	 	$f = new Folder();
	 	$aFolders = $f->getAllUserFolders();

	 	return View::make('client.folders.overview',array(
	 		'title' => 'Mappen',
	 		'folders' => $aFolders
	 	));
	 }))->before('auth');
	 	...
	 	Route::get('client/folder/add', function() {
			...
			foreach ($foldersArray as $folder) {
		 	...
		 	return View::make('client.folders.add',array(
	 	...
	 	Route::get('client/folder/edit/{id}', function($id) {
			...
			return View::make('client.folders.edit', array(
				'title' => 'Map bewerken',
				'folder' => $folder,
				'folders' => $aFolders,
			));
		...
		Route::get('client/folder/delete/{id}', function($id) {
			if (Files::where('fid', '=', $id)->count() > 1) {
				...
				return View::make('client.folders.delete', array(
