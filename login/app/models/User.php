...
class User extends Eloquent implements UserInterface, RemindableInterface {
	...
	static public function getUserUsername($uid) {
		$u = new User();
		$u = $u->where('id', '=', $uid);
	...
	static public function getOrganizationName() {
		if (Auth::user()->oid != 0) {
			$u = new User();
	...
	static public function getClientName() {
		if (Auth::user()->cid != 0) {
			$u = new User();
	...
	static public function getFirstUser($oid,$cid,$username = false) {
	    if ($username) {
	        $user = new User();
...
// 		dd(DB::getQueryLog());
	...
	static public function getAllUsers($oid,$cid,$username = false) {
	    if ($username) {
	        $user = new User();
	        $user = $user->where('rights', '=', '1')->where('oid', '=', $oid)->where('cid', '=', $cid)->whereRaw("(name like '%".$username."%' OR username like '%".$username."%')");
	        if ($user->count() > 0) { 
	...
	static public function checkBilling() {
